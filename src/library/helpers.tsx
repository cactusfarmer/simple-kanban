...
const getAllOtherBoards = (boards: BoardData[], boardId: number) => boards.filter(({ id }) => id !== boardId)
const getOtherColumns = (columns: ColumnData[], columnId: number) => columns.filter(({ id }) => id !== columnId)
const getOtherCards = (cards: CardData[], cardId: number) => cards.filter(({ id }) => id !== cardId)

const getBoard = (boards: BoardData[], boardId: number) => {
    const board = boards.find(({ id }) => { return id === boardId })
...
export { logDiff, logObj, getAllOtherBoards, getOtherColumns, getOtherCards, getBoard, getColumn, getCard }